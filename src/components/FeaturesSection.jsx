import { PopIn, FadeInUp } from './AnimatedSection';
  ...
  return (
    <section className="py-24 border-b-8 border-black bg-slate-50 px-4" aria-label="計畫三大特色">
      <div className="max-w-6xl mx-auto grid sm:grid-cols-2 md:grid-cols-3 gap-8 text-center text-black">
        {features.map((f, i) => (
          <PopIn key={f.title} delay={i * 0.12}>
            <article className={`${f.color} border-4 border-black p-10 comic-shadow transform hover:-translate-y-4 transition-transform h-full`}>
              <div
